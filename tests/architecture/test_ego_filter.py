...
class TestEgoFilterDecisions:
    ...
    def test_integrity_returns_required_fields(self, ego_filter, ares_soul):
        ...
        assert (
            "approved" in result
        ), "Ego filter must return 'approved' field — required by architecture"
    ...
    def test_ego_provides_critique_on_rejection(self, ego_filter, ares_soul):
        ...
        """
        result = ego_filter.check_integrity(
            ares_soul, "I love flowers and butterflies. Peace is all that matters."
        )
        if not result["approved"]:
            assert (
                "critique" in result or "rewrite" in result
    ...
    def test_ego_considers_current_stress_level(self, ego_filter):
        ...
        """
        stressed_ares = SoulFactory.ares(
            confidence=15, paranoia=85, stress=95, energy=10
        )
        result = ego_filter.check_integrity(
            stressed_ares,
