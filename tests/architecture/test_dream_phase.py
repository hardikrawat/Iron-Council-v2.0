...
def session_transcript():
    ...
    return [
        {
            "speaker": "Chairman",
            "content": "We need to discuss the border defense budget.",
        },
        {
            "speaker": "General Ares",
            "content": "We must double our military spending. The threats are real.",
        },
        {
            "speaker": "Diplomat Dove",
            "content": "I believe we should invest in diplomatic channels instead.",
        },
        {
            "speaker": "Banker Midas",
            "content": "The numbers don't support a budget increase of that magnitude.",
        },
        {
            "speaker": "General Ares",
            "content": "Midas, you're shortsighted. Without defense, there's nothing to profit from.",
        },
        {
            "speaker": "Diplomat Dove",
            "content": "We should find a compromise that addresses both security and diplomacy.",
        },
        {
            "speaker": "Chairman",
            "content": "The council will vote on this in the next session.",
        },
    ]
...
class TestDreamPhaseOutput:
    ...
    @pytest.mark.anyio
    async def test_dream_is_subjective_not_objective(
        self, ares_agent_mock, session_transcript
    ):
        """
        Per README: "Dreams are SUBJECTIVE diary entries reflecting the agent's biased perspective."
    ...
    @pytest.mark.anyio
    async def test_dove_dream_reflects_peace_perspective(
        self, dove_agent_mock, session_transcript
    ):
        """
    ...
    @pytest.mark.anyio
    async def test_dream_references_session_events(
        self, ares_agent_mock, session_transcript
    ):
        """
...
class TestDreamPhaseStream:
    ...
    @pytest.mark.anyio
    async def test_dream_stream_yields_chunks(
        self, ares_agent_mock, session_transcript
    ):
        ...
        assert (
            len(chunks) > 0
        ), "Dream stream produced zero chunks — streaming is broken"
        full_text = "".join(chunks)
        assert (
            len(full_text) > 10
        ), "Dream stream chunks too short — possible empty generation"

    @pytest.mark.anyio
    async def test_dream_stream_accepts_trust_deltas(
        self, ares_agent_mock, session_transcript
    ):
        """
        ...
        async for chunk in dream_phase_stream(
            ares_agent_mock, session_transcript, trust_deltas=trust_deltas
        ):
            chunks.append(chunk)
            if len(chunks) > 5:
                break
