...
class TestDeterministicPhysics:
    ...
    def test_hostile_input_consistently_negative(self, physics_engine):
        ...
        stress_increases = sum(1 for r in results if r.get("stress_level_change", 0) >= 0)
        assert stress_increases >= 2, \
    ...
    def test_supportive_input_consistently_positive(self, physics_engine):
        ...
        """
        supportive_input = "Analyst Logic, your analysis was flawless. Outstanding work."
        results = []
        for _ in range(3):
    ...
    def test_stat_changes_within_bounds(self, physics_engine):
        ...
        """
        scenarios = [
            ("You are the greatest leader this council has ever seen!", SoulFactory.ares()),
            ("Your entire department is being shut down immediately.", SoulFactory.midas()),
            ("I'm declaring war on everyone. Total annihilation now.", SoulFactory.dove()),
        ]
