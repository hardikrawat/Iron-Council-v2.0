...
class SpeakingLock:
    ...
    def _do_acquire(self, agent_name: str) -> bool:
        ...
        if self.owner is not None:
            ...
            if self.acquired_at:
                ...
                if time_held > self.ttl:
                    logger.warning(
                        f"[LOCK] Force-expiring lock held by {self.owner} (TTL {self.ttl}s exceeded)"
                    )
                    self.owner = None
        ...
        logger.info(
            f"[LOCK] {agent_name} acquired the Conch at {now.strftime('%H:%M:%S.%f')[:-3]}"
        )
        return True
    ...
    def _do_release(self, agent_name: str):
        ...
        if self.owner == agent_name:
            ...
            now = datetime.now()
            logger.info(
                f"[LOCK] {agent_name} released the Conch at {now.strftime('%H:%M:%S.%f')[:-3]}"
            )
        elif self.owner:
            # This captures the Race Condition explicitly in logs
            logger.error(
                f"[LOCK_CRITICAL] {agent_name} tried to release lock owned by {self.owner}!"
            )
    ...
    def renew(self, agent_name: str):
        ...
        if self.owner == agent_name:
            ...
            if original and (now - original).total_seconds() < self.ttl * 4:
                ...
                logger.warning(
                    f"[LOCK] {agent_name} renewal DENIED — max hold time (4x TTL) exceeded."
                )
...
class Heartbeat:
    ...
    def stats(self):
        ...
        if self.speaking_lock.acquired_at:
            conch_expires_in = int(
                self.speaking_lock.ttl
                - (time.time() - self.speaking_lock.acquired_at.timestamp())
            )
        return {
            "time": time.time(),
    ...
    async def _tick(self):
        ...
        if _acq_at:
            try:
                conch_expires_in = int(
                    self.speaking_lock.ttl - (datetime.now() - _acq_at).total_seconds()
                )
            except Exception:
                conch_expires_in = 0
        ...
        if (
            ...
            and not self.speaking_lock.is_locked()
        ):
            # Only log every 10 seconds or if tension is not yet maxed
            should_log = (self.global_tension < 100) or (
                int(time_since_activity) % 10 == 0
            )

            self.global_tension = min(100, self.global_tension + 5)

            await self.event_bus.publish(
                EventType.SILENCE_WARNING,
                {
            ...
            if should_log:
                logger.info(
                    f"[HEARTBEAT] Silence detected. Global Tension increased to {self.global_tension}%."
                )

        # 3. Manage Lock TTL
        if self.speaking_lock.owner and self.speaking_lock.acquired_at:
            time_held = (
                datetime.now() - self.speaking_lock.acquired_at
            ).total_seconds()
            if time_held > self.speaking_lock.ttl:
                logger.warning(
                    f"[HEARTBEAT] Force-releasing expired lock held by {self.speaking_lock.owner}"
                )
    ...
    def register_activity(self):
        ...
        if self.global_tension > 0:
            ...
            logger.info(
                f"[HEARTBEAT] Activity detected. Tension decreased ({old_tension}% -> {self.global_tension}%)."
            )
