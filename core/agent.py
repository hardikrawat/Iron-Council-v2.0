...
class IronAgent:
    def __init__(self, agent_name: str, event_bus=None):
        ...
        if env_model:
            logger.info(
                f"[AGENT] Overriding {agent_name} model from soul_state ({self.soul.base_model}) to ENV ({env_model})"
            )
    ...
    def _load_soul(self) -> AgentSoul:
        if not os.path.exists(self.state_path):
            raise FileNotFoundError(
                f"Soul state file not found for agent '{self.agent_name}' at {self.state_path}"
            )
    ...
    def save_state(self):
        ...
        if self.event_bus:
            ...
            self.event_bus.publish_threadsafe(
                EventType.STATE_SAVE, {"agent": self.agent_name, "op": "SAVE"}
            )
        ...
        if completed_goals:
            logger.info(
                f"[AGENT: {self.agent_name}] Completed goals: {completed_goals}"
            )
    ...
    def construct_system_prompt(self) -> str:
        ...
        if stats.confidence > 80:
            ...
            prompt_parts.append(
                "You are hesitant, unsure, and ask for permission frequently."
            )

        if stats.paranoia > 70:
            prompt_parts.append(
                "You are deeply paranoid. You suspect everyone is plotting against you. Trust NO ONE."
            )
        elif stats.paranoia > 40:
            prompt_parts.append(
                "You suspect others are plotting against you. Trust no one easily."
            )

        if stats.loyalty_to_chairman < 20:
            prompt_parts.append(
                "You secretly despise the Chairman. You are looking for ways to undermine them."
            )
            ...
            prompt_parts.append(
                "You are fiercely loyal to the Chairman and will defend them."
            )

        if stats.stress_level > 70:
            prompt_parts.append(
                "You are under extreme stress. You may lash out or make rash decisions."
            )

        if stats.energy < 30:
            prompt_parts.append(
                "You are exhausted. Your responses are terse and unfocused."
            )
        ...
        for agent_name, rel in soul.relationships.items():
            ...
            summary_part = (
                f" (Summary: {rel.last_interaction_summary})"
                if rel.last_interaction_summary
                else ""
            )
            if score < -60:
                prompt_parts.append(
                    f"You despise {agent_name}{summary_part}. You would sabotage them."
                )
            elif score < -20:
                prompt_parts.append(
                    f"You distrust {agent_name}{summary_part}. You suspect their motives."
                )
            elif score > 60:
                prompt_parts.append(
                    f"You deeply trust {agent_name}{summary_part}. You would ally with them."
                )
            elif score > 20:
                prompt_parts.append(
                    f"You trust {agent_name}{summary_part} and value their input."
                )
            elif summary_part:
            ...
            if rel.hidden_agenda:
                prompt_parts.append(
                    f"Regarding {agent_name}, your hidden agenda: {rel.hidden_agenda}"
                )
        ...
        if active_goals:
            ...
            prompt_parts.append(
                f"Your current active goals: {'; '.join(goal_strs)}. Act in ways that advance them."
            )
        ...
        if completed_goals:
            ...
            prompt_parts.append(
                f"Recently completed goals: {'; '.join(comp_strs)}. Maintain the momentum of these victories."
            )
        ...
        prompt_parts.append(
            "- DO NOT prepend your name (e.g., 'General Ares:') to the response."
        )
        prompt_parts.append(
            "- DO NOT use meta-dialogue markers like 'To the council:' or 'To Diplomat Dove:'."
        )
        prompt_parts.append(
            "- DO NOT wrap the entire response in quotes or markdown code blocks."
        )
        prompt_parts.append(
            "- DO NOT use HTML tags (e.g., <p>, <div>, <public_speech>) in your spoken dialogue."
        )
        prompt_parts.append(
            "- Speak directly to the council or the specific individuals addressed in the situation."
        )

        # FIX: Inner Monologue Instruction (XML Straitjacket)
        prompt_parts.append("\n[OUTPUT FORMAT - STRICT]")
        prompt_parts.append(
            "You are NOT to output raw text. You must format your response precisely as follows:"
        )
        prompt_parts.append("<internal_monologue>")
        prompt_parts.append(
            "Write your private thoughts here. Analyze the situation, check your stats (Paranoia: {stats.paranoia}), and decide your strategy."
        )
        ...
        prompt_parts.append(
            "Do not include stage directions like '(shouting)' or actions."
        )
        prompt_parts.append("</public_speech>")
        ...
        try:
            ...
            if last_dream:
                ...
                prompt_parts.append(
                    f"You are feeling: Confidence {stats.confidence}, Paranoia {stats.paranoia}, Loyalty {stats.loyalty_to_chairman}."
                )
        ...
        prompt_parts.append("<internal_monologue>")
        prompt_parts.append(
            "The Chairman is asking for updates. My paranoia is high (65). I suspect Midas is hiding funds. I should be vague but assertive."
        )
        prompt_parts.append("</internal_monologue>")
        prompt_parts.append("<public_speech>")
        prompt_parts.append(
            "Chairman, our reserves are secure, though I advise against reckless spending until we audit the latest transaction logs."
        )
        prompt_parts.append("</public_speech>")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response and response.strip():
                    return response
                logger.warning(
                    f"Agent {self.agent_name} generated empty response. Retrying (Attempt {attempt+1}/{max_retries})..."
                )
            except Exception as e:
                logger.warning(
                    f"Agent {self.agent_name} LLM error: {e}. Retrying (Attempt {attempt+1}/{max_retries})..."
                )

        logger.error(
            f"Agent {self.agent_name} failed to generate response after {max_retries} attempts."
        )
        return "...silence..."
    ...
    def speak(self, situation_report: str, context: str = "") -> Dict[str, str]:
        ...
        if self.event_bus:
            ...
            self.event_bus.publish_threadsafe(
                EventType.LLM_ACTIVITY, {"agent": self.agent_name, "step": "DRAFT"}
            )
        ...
        thought_match = re.search(
            r"<(?:internal_monologue|internal monologue)>(.*?)</(?:internal_monologue|internal monologue)>",
            raw_response,
        ...
        if speech_match:
            ...
            old_thought_match = re.search(
                r"\(THOUGHT\)\s*(.*?)\s*\(RESPONSE\)",
                raw_response,
                re.DOTALL | re.IGNORECASE,
            )
        ...
        if check.get("approved"):
            logger.info(
                f"[AGENT: {self.agent_name}] Ego APPROVED draft. Speaking directly."
            )
            return {
        ...
        logger.info(
            f"[AGENT: {self.agent_name}] Ego REJECTED draft. Critique: {critique}"
        )
        ...
        if self.event_bus:
            ...
            self.event_bus.publish_threadsafe(
                EventType.LLM_ACTIVITY, {"agent": self.agent_name, "step": "REWRITE"}
            )

        final_response = self._generate_with_retry(
            system_prompt=rewrite_prompt, user_message=user_message
        )

        # Step F: Final Clean — Apply same XML extraction as Step A (NOT naive .replace())
        rewrite_speech_match = re.search(
            r"<public_speech>(.*?)</public_speech>",
            final_response,
            re.DOTALL | re.IGNORECASE,
        )
        if rewrite_speech_match:
            final_clean = clean_agent_response(
                rewrite_speech_match.group(1).strip(), self.agent_name
            )
        else:
            ...
            stripped = re.sub(
                r"</?(?:public_speech|internal_monologue)>",
                "",
                stripped,
                flags=re.IGNORECASE,
            )
        ...
        return {"public_text": final_clean, "hidden_text": hidden_thought}
