...
class PhysicsSystem:
    ...
    def __init__(
        self,
        event_bus: EventBus,
        physics: GamemasterPhysics,
        agents: List[IronAgent],
    ...
    async def on_world_event(self, payload: Dict[str, Any]):
        ...
        await self._append_log(
            {"type": "user", "content": content, "timestamp": timestamp}
        )
        ...
        for agent in self.agents:
            task = asyncio.create_task(
                self._process_world_event_for_agent(agent, content)
            )
    ...
    async def _process_world_event_for_agent(self, agent: IronAgent, content: str):
        ...
        try:
            ...
            await self.event_bus.publish(
                EventType.PHYSICS_SYNC,
                {"status": "START", "agent": agent.agent_name, "type": "WORLD_IMPACT"},
            )

            # Physics Calculation - Non-blocking
            impact = await asyncio.to_thread(
                self.physics.calculate_impact, content, agent.soul
            )
            ...
            agent.soul.update_stat(
                "loyalty_to_chairman", impact.get("loyalty_to_chairman_change", 0)
            )
            ...
            try:
                ...
                logger.debug(
                    f"[MEMORY] Saved World Event to {agent.soul.name}'s memory."
                )
            except Exception as mem_err:
                logger.error(
                    f"[MEMORY] Failed to save World Event for {agent.soul.name}: {mem_err}"
                )
            ...
            await self.event_bus.publish(
                EventType.PHYSICS_COMPLETE,
                {"agent": agent.agent_name, "type": "WORLD_EVENT", "content": content},
            )

            elapsed = time.time() - start_time
            logger.info(
                f"[PHYSICS] Updated {agent.soul.name} stats via World Event. (Latency: {elapsed:.2f}s)"
            )
    ...
    async def on_agent_speak(self, payload: Dict[str, Any]):
        ...
        await self._append_log(
            {"type": "agent_post", "data": entry_data, "timestamp": timestamp}
        )
        ...
        speaker_agent = next(
            (a for a in self.agents if a.agent_name == speaker_name), None
        )
        if speaker_agent:
            try:
                speaker_agent.memory.save_memory(
                    agent_name=speaker_name, text=f"I said: {content}", emotion="speech"
                )
            except Exception as mem_err:
                logger.error(
                    f"[MEMORY] Failed to save self-speech for {speaker_name}: {mem_err}"
                )
        ...
        for listener in self.agents:
            ...
            # Queue reaction task
            reaction_tasks.append(
                self._process_reaction(listener, speaker_name, content)
            )
    ...
    async def flush_all(self):
        ...
        if self.pending_tasks:
            logger.info(
                f"[PHYSICS] Waiting for {len(self.pending_tasks)} pending reaction tasks..."
            )
    ...
    async def _process_reaction(
        self, listener: IronAgent, speaker_name: str, content: str
    ):
        import time
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"[MEMORY] Failed to save peer-speech for {listener.soul.name}: {mem_err}"
                )
            ...
            if delta != 0:
                await self.event_bus.publish(
                    EventType.AGENT_STATUS,
                    {
                        ...
                        "relationship_data": {
                            ...
                            "delta": delta,
                            "new_score": listener.soul.get_relationship_score(
                                speaker_soul_name
                            ),
                        },
                    },
                ...
                logger.info(
                    f"[PHYSICS] {listener.soul.name} had no reaction to {speaker_soul_name}. (Latency: {elapsed:.2f}s)"
                )
    ...
    async def _run_gamemaster_loop(self):
        ...
        deltas = await asyncio.to_thread(
            self.physics.adjudicate_narrative, buffer_snapshot, active_goals
        )
        ...
        for agent_soul_name, goal_impacts in deltas.items():
            # Find the agent
            agent = next(
                (a for a in self.agents if a.soul.name == agent_soul_name), None
            )
            if not agent:
                continue
            ...
            for goal_key, impact_data in goal_impacts.items():
                ...
                target_goal = next(
                    (
                        g
                        for g in agent.soul.goals
                        if g.active and g.description == goal_key
                    ),
                    None,
                )
                if not target_goal:
                    ...
                    target_goal = next(
                        (
                            g
                            for g in agent.soul.goals
                            if g.active and goal_key in g.description
                        ),
                        None,
                    )
                ...
                if target_goal:
                    ...
                    if final_delta != 0:
                        agent.soul.update_goal_progress(
                            target_goal.description, final_delta
                        )
                        updates_made = True
                        logger.info(
                            f"[GAMEMASTER] {agent.soul.name} Goal '{target_goal.description}': {raw_delta} -> {final_delta} | REASON: {reason}"
                        )
