...
class OODALoop:
    def __init__(self, agent: IronAgent, event_bus: EventBus, heartbeat: Heartbeat):
        ...
        self._running = False
        self._last_processed_world_event = (
            None  # Fix #3: track processed chairman messages
        )
        self._last_processed_agent_event = (
            None  # Fix #16: track processed agent messages
        )
        ...
        self._drain_gate = (
            False  # FIX BUG-C: Gate to suppress speech after drain timeout
        )
        ...
        self._sub_silence = partial(
            self._on_event, event_type=EventType.SILENCE_WARNING
        )
        self._sub_status = partial(self._on_event, event_type=EventType.AGENT_STATUS)
        self._sub_physics = partial(
            self._on_event, event_type=EventType.PHYSICS_COMPLETE
        )
    ...
    async def _on_event(self, payload: Dict[str, Any], event_type: str = None):
        ...
        if event_type == EventType.WORLD_EVENT:
            ...
            logger.info(
                f"{self.agent.soul.name} detected World Event. Waiting for emotional impact..."
            )
        ...
        if (
            event_type == EventType.PHYSICS_COMPLETE
            or event_type == EventType.AGENT_STATUS
        ):
            # Check for Physics complete OR Relationship Update
            is_physics = (
                event_type == EventType.PHYSICS_COMPLETE
                and event.get("agent") == self.agent.agent_name
            )
            is_rel_update = (
                event_type == EventType.AGENT_STATUS
                and event.get("status") == "RELATIONSHIP_UPDATE"
                and event.get("agent") == self.agent.agent_name
            )
            ...
            if is_physics or is_rel_update:
                if self._waiting_for_physics:
                    ...
                    logger.info(
                        f"{self.agent.soul.name} processed physics/relationship update. Ready to respond."
                    )
    ...
    def _format_context(self, events: List[Dict]) -> str:
            ...
            if etype == EventType.WORLD_EVENT:
                ...
                if status == "NARRATIVE_VERDICT":
                    ...
                    trend = (
                        "ADVANCED"
                        if delta > 0
                        else "REGRESSED" if delta < 0 else "STAGNATED"
                    )
                    lines.append(
                        f'[INTERNAL SENSE]: Your progress on "{goal}" has {trend} ({delta}%). GM VERDICT: {details}'
                    )
                else:
        ...
        if self.heartbeat.global_tension > 20:
            lines.append(
                f"[Atmosphere: The council is silent. Tension is at {self.heartbeat.global_tension}%.]"
            )
    ...
    async def start(self):
        ...
        while self._running:
            try:
                ...
                logger.error(
                    f"Error in {self.agent.soul.name} OODA loop (Attempt {self._consecutive_errors}): {e}"
                )
                ...
                if self._consecutive_errors > 5:
                    logger.critical(
                        f"{self.agent.soul.name} is COMATOSE due to repeated errors. Sleeping for 60s."
                    )
    ...
    async def wait_for_drain(self, timeout: float = 15.0):
        ...
        try:
            ...
            logger.warning(
                f"{self.agent.soul.name} drain timed out after {timeout}s. Proceeding anyway."
            )
            return False
    ...
    async def _run_cycle(self):
        ...
        if self._waiting_for_physics:
            ...
            if time.time() - self._physics_wait_start > 120.0:
                self._waiting_for_physics = False
                logger.warning(
                    f"{self.agent.soul.name} timed out waiting for physics (120s). Proceeding anyway."
                )
            else:
                await self.event_bus.publish(
                    EventType.AGENT_STATUS,
                ...
                logger.info(
                    f"{self.agent.soul.name} [OODA] -> Gated by Physics. Waiting for emotional impact."
                )
        ...
        if stats.energy < 10:
            ...
            logger.info(
                f"{self.agent.soul.name} [OODA: ORIENT] -> Energy critical ({stats.energy}). Resting to recharge."
            )
        ...
        for e in recent_events:
            if e.get("type") == EventType.WORLD_EVENT:
                ...
                if event_content != self._last_processed_world_event:
                    ...
                    logger.info(
                        f"{self.agent.soul.name} [OODA: DECIDE] -> Triggered by World Event: '{event_content[:50]}...'"
                    )
                ...
                if speaker != self.agent.agent_name:  # Don't reply to self
                    ...
                    if content != self._last_processed_agent_event:
                        ...
                        if random.random() < 0.7:
                            ...
                            logger.info(
                                f"{self.agent.soul.name} [OODA: DECIDE] -> Triggered by peer {speaker}: '{content[:50]}...'"
                            )
                            break
                        else:
                            logger.info(
                                f"{self.agent.soul.name} [OODA: DECIDE] -> Ignored peer {speaker} (Randomness check)."
                            )

        # Heuristic B: High Tension/Paranoia?
        if not should_think and (
            self.heartbeat.global_tension > 50 or stats.paranoia > 80
        ):
            if random.random() < 0.4:  # 40% chance to react to tension
                should_think = True
                decision_trigger = ("TENSION", "High Tension")
                logger.info(
                    f"{self.agent.soul.name} [OODA: DECIDE] -> Triggered by High Tension ({self.heartbeat.global_tension}%) or Paranoia."
                )
        ...
        if self._current_decision_trigger == decision_trigger and self._cached_context:
            ...
            logger.info(
                f"{self.agent.soul.name} [OODA: DECIDE] -> Reusing cached context for ongoing trigger."
            )
        else:
            memory_context_str = ""
            ...
            try:
                ...
                if memories:
                    ...
                    logger.info(
                        f"{self.agent.soul.name} [OODA: DECIDE] -> Recalled {len(memories)} memories."
                    )
                else:
                    logger.debug(
                        f"{self.agent.soul.name} [OODA: DECIDE] -> No memories found."
                    )
            except Exception as mem_err:
                logger.warning(
                    f"Memory recall failed for {self.agent.soul.name}: {mem_err}"
                )
        ...
        if acquired:
            try:
                ...
                if self._drain_gate:
                    logger.warning(
                        f"Drain gate active for {self.agent.soul.name}. Suppressing AGENT_SPEAK broadcast."
                    )
                else:
